...
class Room {
   ...
   * sin importar cuánto tiempo haya pasado: el cartón de un jugador nunca
   * se libera solo, solo cuando el host reinicia la sala.
   */
  ...
  /** Marca a un jugador como desconectado (sin soltarle el cartón ni borrarlo). */
  markPlayerDisconnected(clientId, socketId) {
    const player = this.players.get(clientId);
   ...
   * de la sala. Sin importar cuánto tiempo haya pasado: la sala nunca se
   * cierra sola, solo con el botón "Reiniciar sala".
   */
  ...
  /** Marca al host como desconectado (sin cerrar la sala). */
  markHostDisconnected(socketId) {
  ...
  /**
   * Marca automáticamente, para un jugador, todas las celdas de su cartón
   * cuyo número ya salió y todavía no había marcado — se usa al reconectar
   * (refresh, corte de wifi, celular que se apagó) para que no tenga que
   * ponerse a marcar uno por uno a mano lo que se perdió mientras no
   * estaba. Reutiliza markCell celda por celda, así que si al ponerse al
   * día completa una terna/línea/cartón, el premio se detecta igual.
   */
  autoMarkDrawnNumbers(clientId) {
    const player = this.players.get(clientId);
    if (!player || !player.card) return { ok: true, player, wins: [] };
    ...
    for (let row = 0; row < 3; row++) {
      for (let col = 0; col < 9; col++) {
        ...
        const key = `${row}-${col}`;
        if (player.marked.has(key)) continue;
        if (!this.drawnNumbers.includes(value)) continue;
        const result = this.markCell(clientId, row, col);
        if (result.ok) wins.push(...result.wins);
      }
    }
    return { ok: true, player, wins };
  }
